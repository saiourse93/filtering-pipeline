...
def runBowtie2(inputFastq1, inputFastq2, targetDB, outputBowtie, bowtieParams, tmpDir, nodesToUse,
    ...
    if inputFastq2:
        ...
        cmd = "%s %s -p %s --un %s -x %s -U %s -S %s/%s" % (config.PATH_TO_BOWTIE2,
                                                        bowtieParams, nodesToUse, outputUnaligned, targetDB,
                                                        inputFastq1, tmpDir, outputBowtie) # Original line: "inputFastq1, inputFastq2, tmpDir, outputBowtie" 
...
def runBlast(inputFasta, targetDB, outputBlast, params, tmpDir, nodesToUse, blastLogFile, mylistener):
    ...
    if config.USE_GRID:
        ...
        runStraightBlast(inputFasta, targetDB, outputBlast, params, nodesToUse, mylistener) # Phele: Added "nodesToUse"
...
def runStraightBlast(inputFasta, targetDB, outputBlast, params, nodesToUse, mylistener): # Phele: Added "nodesToUse"
    if mylistener:
        mylistener.reportImportantInfo("Running blast.  Variable values:%s" % (getArgString(inspect.currentframe())))
              
    cmd = "%s -outfmt 6 %s -num_threads %s -db %s -query %s -out %s" % (config.PATH_TO_BLASTN, params, nodesToUse, targetDB, inputFasta, outputBlast) # Phele: Added -num_threads %s and nodesToUse
...
def filterUsingBlast(inputFasta, blastResults, minPercent=None, minLength=None, minNumIds=None,
                     maxEval=None, minBitScore=None, delimiter=' ', pairedHits=False, pairedMisses=True, # Phele: Changed delimiter from '#0/' to ' ' 
...
def getFastaForTaxonomy(taxIdList, blastInputGlob, fastaInputGlob,  fastaOutput, printLineageOfHits=False):
    ...
    usedTaxonomies = set()
    print "Getting taxonomy ids from local file for %s gis" % (len(gis))
    for line in open(config.BLAST_TAX_DB): #PHELE
        (gi, taxid) = line.split()
        if gis.has_key(gi):
